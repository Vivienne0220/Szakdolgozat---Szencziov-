...
{
    public class Ostatne : Form
    {
        private IMongoCollection<Termekek> ostatneTabla = Program.termekAdatbazis.GetCollection<Termekek>("ostatne");

        private DataGridView dataOstatneB;
        ...
        {
            this.Text = "Ostatné - Pracovník";
            this.Size = new System.Drawing.Size(1100, 450);
            this.FormBorderStyle = FormBorderStyle.FixedSingle;
            this.MaximizeBox = false;

            dataOstatneB = new DataGridView
            {
            ...
            this.Controls.Add(dataOstatneB);
        ...
        private void TextboxKod_TextChanged(object? sender, EventArgs e)
        {
            ...
            var termek = ostatneTabla.Find(filterDefinition).FirstOrDefault();
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            ...
            ostatneTabla.UpdateOne(filterDefinition, updateDefinition);
            refreshDataGrid();
        }
        ...
        private void InitializeComponentB()
        {
            ...
            {
                Text = $"Spolu ostatné: {celkomostatne.ToString("N2")}",
        ...
        private void buttonNewB_Click(object sender, EventArgs e)
        {
            ...
            ostatneTabla.InsertOne(newTermek);
            refreshDataGrid();
        }
        ...
        {
            var filterDefinition = Builders<Termekek>.Filter.Eq(a => a.ProductCode, textboxKodB.Text);
            ostatneTabla.DeleteOne(filterDefinition);
        ...
        private void refreshDataGrid()
        {
            ...
            var term = ostatneTabla.Find(filterDefinition).ToList();
            ...
            foreach (var termek in term)
            {
                ...
                decimal ostatne = (((termek.ZosPred + termek.Prijem) - termek.UzavZos) * termek.Price);

                if (_isAdmin)
                {
                    row["Spolu"] = ostatne.ToString();
                }
                ;

                celkomostatne += ostatne;

                dataTable.Rows.Add(row);
            }
            if (_isAdmin)
            {
                labelCelkomB.Text = $"Spolu ostatne: {celkomostatne.ToString("N2")} €";
            }

            dataOstatneB.DataSource = dataTable;
        }
